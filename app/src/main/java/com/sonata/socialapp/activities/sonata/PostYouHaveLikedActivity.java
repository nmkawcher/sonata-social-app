...
public class PostYouHaveLikedActivity extends AppCompatActivity implements RecyclerViewClick {
    ...
    private void getAds(List<Post> objects,boolean isRefresh){
        ...
        if(GenelUtil.isAlive(PostYouHaveLikedActivity.this)){
            ...
            if(c<=0){
                ...
                AdLoader adLoader = new AdLoader.Builder(PostYouHaveLikedActivity.this, getString(R.string.adId))
    ...
    public void onReloadImageClick(int position, RoundKornerRelativeLayout reloadLayout, ProgressBar progressBar, ImageView imageView) {
        ...
        if(post.getNsfw()){
            Glide.with(PostYouHaveLikedActivity.this).load(post.getMainMedia().getUrl()).apply(RequestOptions.bitmapTransform(new BlurTransformation(25, 3))).addListener(new RequestListener<Drawable>() {
                @Override
            ...
            if(post.getRatioH()>1280||post.getRatioW()>1280){
                ...
                Glide.with(PostYouHaveLikedActivity.this).load(post.getMainMedia().getUrl()).override(iw,ih).thumbnail(Glide.with(PostYouHaveLikedActivity.this).load(post.getThumbMedia().getUrl())).addListener(new RequestListener<Drawable>() {
                    @Override
                    public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Drawable> target, boolean isFirstResource) {
                        progressBar.setVisibility(View.INVISIBLE);
                        reloadLayout.setVisibility(View.VISIBLE);
                ...
                Glide.with(PostYouHaveLikedActivity.this).load(post.getMainMedia().getUrl()).override(Target.SIZE_ORIGINAL, Target.SIZE_ORIGINAL).thumbnail(Glide.with(PostYouHaveLikedActivity.this).load(post.getThumbMedia().getUrl())).addListener(new RequestListener<Drawable>() {
                    @Override
                    public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Drawable> target, boolean isFirstResource) {
                        progressBar.setVisibility(View.INVISIBLE);
                        reloadLayout.setVisibility(View.VISIBLE);
