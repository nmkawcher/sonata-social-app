...
public class HashtagActivity extends AppCompatActivity implements RecyclerViewClick {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        title = findViewById(R.id.profileusernametext);
    ...
    private void getAds(List<Post> objects,boolean isRefresh){
        ...
        if(GenelUtil.isAlive(HashtagActivity.this)){
            ...
            if(c<=0){
                ...
                AdLoader adLoader = new AdLoader.Builder(HashtagActivity.this, getString(R.string.adId))
    ...
    public void onOptionsClick(int position, TextView commentNumber) {
        ...
        ProgressDialog progressDialog = new ProgressDialog(HashtagActivity.this);
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(HashtagActivity.this);
            ...
            public void onClick(DialogInterface dialog, int which) {
                    ...
                    ParseCloud.callFunctionInBackground("enableDisableComment", params, new FunctionCallback<String>() {
                        ...
                        public void done(String object, ParseException e) {
                            if(e==null){
                                ...
                                commentNumber.setText(HashtagActivity.this.getString(R.string.disabledcomment));
                            }
                            else{
                                progressDialog.dismiss();
                                GenelUtil.ToastLong(HashtagActivity.this,getString(R.string.error));
                            }
                        }
                    });
                    ...
                    ParseCloud.callFunctionInBackground("savePost", params, new FunctionCallback<String>() {
                        ...
                        public void done(String object, ParseException e) {
                            if(e==null){
                                post.setSaved(true);
                                progressDialog.dismiss();
                                GenelUtil.ToastLong(HashtagActivity.this,getString(R.string.postsaved));
                    ...
                    ParseCloud.callFunctionInBackground("unsavePost", params, new FunctionCallback<String>() {
                        ...
                        public void done(String object, ParseException e) {
                            if(e==null){
                                post.setSaved(false);
                                progressDialog.dismiss();
                                GenelUtil.ToastLong(HashtagActivity.this,getString(R.string.postunsaved));
                ...
                if(select.equals(getString(R.string.report))){
                    ...
                    ParseCloud.callFunctionInBackground("reportPost", params, new FunctionCallback<String>() {
                        ...
                        public void done(String object, ParseException e) {
                            if(e==null){
                                GenelUtil.ToastLong(HashtagActivity.this,getString(R.string.reportsucces));
                                progressDialog.dismiss();
                            }
                            else{
                                progressDialog.dismiss();
                    ...
                    ParseCloud.callFunctionInBackground("enableDisableComment", params, new FunctionCallback<String>() {
                        ...
                        public void done(String object, ParseException e) {
                            if(e==null){
                                ...
                                commentNumber.setText(GenelUtil.ConvertNumber((int)post.getCommentnumber(),HashtagActivity.this));
                            }
                            else{
                                progressDialog.dismiss();
                                GenelUtil.ToastLong(HashtagActivity.this,getString(R.string.error));
    ...
    public void onSocialClick(int position, int clickType, String text) {
        if(clickType== HomeAdapter.TYPE_HASHTAG){
            //hashtag
            startActivity(new Intent(HashtagActivity.this, HashtagActivity.class).putExtra("hashtag",text.replace("#","")));
            ...
            if(!username.equals(ParseUser.getCurrentUser().getUsername())){
                startActivity(new Intent(HashtagActivity.this, GuestProfileActivity.class).putExtra("username",username));
            }
            ...
            customTabsIntent.launchUrl(HashtagActivity.this, Uri.parse(url));
        }
    }
    ...
    public void onLikeClick(int position, ImageView likeImage, TextView likeNumber) {
        ...
        if(!post.getLiked()){
            ...
            likeImage.setImageDrawable(HashtagActivity.this.getDrawable(R.drawable.ic_like_red));
            HashMap<String, Object> params = new HashMap<String, Object>();
            params.put("postID", post.getObjectId());
            ...
            post.setLiked(false);
            likeImage.setImageDrawable(HashtagActivity.this.getDrawable(R.drawable.ic_like));
    ...
    public void onGoToProfileClick(int position) {
        Post post = list.get(position).getPost();
        if(!post.getUser().getObjectId().equals(ParseUser.getCurrentUser().getObjectId())){
            startActivity(new Intent(HashtagActivity.this, GuestProfileActivity.class).putExtra("user",post.getUser()));
        }
    }
    ...
    public void onReloadImageClick(int position, RoundKornerRelativeLayout reloadLayout, ProgressBar progressBar, ImageView imageView) {
        ...
        if(post.getNsfw()){
            Glide.with(HashtagActivity.this).load(post.getMainMedia().getUrl()).apply(RequestOptions.bitmapTransform(new BlurTransformation(25, 3))).addListener(new RequestListener<Drawable>() {
                @Override
            ...
            if(post.getRatioH()>1280||post.getRatioW()>1280){
                ...
                Glide.with(HashtagActivity.this).load(post.getMainMedia().getUrl()).override(iw,ih).thumbnail(Glide.with(HashtagActivity.this).load(post.getThumbMedia().getUrl())).addListener(new RequestListener<Drawable>() {
                    @Override
                    public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Drawable> target, boolean isFirstResource) {
                        progressBar.setVisibility(View.INVISIBLE);
                        reloadLayout.setVisibility(View.VISIBLE);
                ...
                Glide.with(HashtagActivity.this).load(post.getMainMedia().getUrl()).override(Target.SIZE_ORIGINAL, Target.SIZE_ORIGINAL).thumbnail(Glide.with(HashtagActivity.this).load(post.getThumbMedia().getUrl())).addListener(new RequestListener<Drawable>() {
                    @Override
                    public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Drawable> target, boolean isFirstResource) {
                        progressBar.setVisibility(View.INVISIBLE);
                        reloadLayout.setVisibility(View.VISIBLE);
